...
def ajustar_tempo_com_filtro(input_dir, output_dir, tempo_file):
    # Ler arquivo de tempos
    tempos_df = pd.read_csv(tempo_file, delimiter=" ", header=None, names=["Pasta", "Tempo Inicial", "Tempo Final"])
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.startswith("recognitions_") and ("_VF" in file or "_VL" in file):
                ...
                df["Posição X (m)"] = pd.to_numeric(df["Posição X (m)"], errors="coerce")
                df["Posição Y (m)"] = pd.to_numeric(df["Posição Y (m)"], errors="coerce")
                
                # Remover linhas com valores NaN
                df.dropna(inplace=True)
                
                # Determinar qual tempo inicial e final usar com base no nome da pasta e tipo de arquivo
                relative_path = os.path.relpath(root, input_dir)
                base_name = file.replace("recognitions_", "").replace(".csv", "")
                matching_rows = tempos_df[tempos_df["Pasta"].str.contains(base_name.split("_VF")[0].split("_VL")[0], na=False)]
                
                if matching_rows.empty:
                    print(f"Aviso: Nenhuma correspondência encontrada para {file}, ignorando...")
                    continue
                
                tempo_inicial = None
                tempo_final = None
                for _, row in matching_rows.iterrows():
                    if ("_VF" in file and "_VF" in row["Pasta"]) or ("_VL" in file and "_VL" in row["Pasta"]):
                        tempo_inicial = float(row["Tempo Inicial"])
                        tempo_final = float(row["Tempo Final"])
                        break
                
                if tempo_inicial is None or tempo_final is None:
                    print(f"Aviso: Nenhuma correspondência exata encontrada para {file}, ignorando...")
                    continue
                
                # Filtrar dados dentro do intervalo de tempo
                df = df[(df["Tempo (s)"] >= tempo_inicial) & (df["Tempo (s)"] <= tempo_final)]
                
                # Ajustar tempo inicial
                df["Tempo (s)"] = df["Tempo (s)"] - tempo_inicial
                df["Tempo (s)"] = df["Tempo (s)"].round(3)
                
                # Garantir que os dados sejam do tipo float64
                df = df.astype({"Tempo (s)": "float64", "Posição X (m)": "float64", "Posição Y (m)": "float64"})
...
def processar_pasta(pasta_input_path, pasta_output_path):
    ...
    ajustar_tempo_com_filtro(pasta_input_path, pasta_output_path,tempo_file)
...
if __name__ == "__main__":
    ...
    tempo_file = "D:/Downloads/video_time-C1.txt"
